# -*- coding: utf-8 -*-
"""
Angel One - cancel option orders
...
def modify_order(order_params):
    response = obj.modifyOrder(order_params)
    return response["data"]["orderid"]

mod_param = order_param.copy()
mod_param["orderid"] = order_id
mod_param["price"] = 200

mod_order_id = modify_order(mod_param)

def cancel_order(order_id, variety="NORMAL"):
    response = obj.cancelOrder(order_id, variety)
    return response

cancel_order(mod_order_id)
