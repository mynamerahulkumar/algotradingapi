# -*- coding: utf-8 -*-
"""
Angel One - Option data streaming (greeks directly from angel one api)
...
for symbol in opt_chain.symbol:
    ...
    option_data[symbol]["expiry_date"] = opt_chain.loc[opt_chain.symbol == symbol, "expiry"].to_list()[0]
...
for option in option_data:
    expiry = option_data[option]["expiry_date"]
    strike = option_data[option]["strike"]
    opt_type = option_data[option]["type"]
    greeks_data = obj.optionGreek({
                                    "name":"BANKNIFTY",
                                    "expirydate":expiry
                                   })
    greeks_data = greeks_data["data"]
    greeks_for_contract = [i for i in greeks_data if float(i["strikePrice"])==strike and i["optionType"]==opt_type][0]
    option_data[option]["imp_vol"] = greeks_for_contract["impliedVolatility"]
    option_data[option]["delta"] = greeks_for_contract["delta"]
    option_data[option]["theta"] = greeks_for_contract["theta"]
    option_data[option]["vega"] = greeks_for_contract["vega"]
    option_data[option]["gamma"] = greeks_for_contract["gamma"]
